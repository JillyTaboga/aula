...
DateTime dateTimeByDateTimeAndTimeOfDay(
...
extension extrasTimeOfDay on TimeOfDay {
...
extension extrasDateTime on DateTime {
  ...
  int lastDay() {
    return (this.month < 12)
        ? new DateTime(this.year, this.month + 1, 0).day
        : new DateTime(this.year + 1, 1, 0).day;
  }
  ...
  DateTime lastDayOfCurrentMonth() {
    ...
    int currentMonth = DateTime.now().month;
    return DateTime(
        currentYear, currentMonth, this.lastDay(), 23, 59, 59, 59, 59);
  }
  ...
  DateTime lastHour() {
    return DateTime(this.year, this.month, this.day, 23, 59, 59, 59, 59);
  }
  ...
  DateTime addDay() {
    ...
    int day = this.day;
    int year = this.year;
    if (this.lastDayOfCurrentMonth().day == this.day) {
      if (this.month == 12) {
        month = 1;
        year++;
        day = 1;
    ...
    return DateTime(year, month, day, this.hour, this.minute, this.second,
...
extension extras on String {
  ...
  double currencyToDouble() => double.tryParse(
      this.replaceAll('R\$', '').replaceAll('.', '').replaceAll(',', '.'));
...
extension extrasDuration on Duration {
  String format() =>
      this?.toString()?.split('.')?.first?.padLeft(8, "0") ?? '00:00:00';
}
